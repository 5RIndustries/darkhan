...
```bash
sudo -u _darkhan /opt/homebrew/bin/node /Users/adminoutlaw/darkhan/server/server.js &
```
...
**On the remote machine (Tino's):**
1. Clone the repo: `git clone https://github.com/outlaw4shrt/darkhan.git`
...
2. Copy `ca.crt` and `remote-node.crt` + `remote-node.key` to Tino's machine
...
bash server/scripts/generate-certs.sh ~/.darkhan-certs my-primary

# Copy new cert to the node if remote
scp ~/.darkhan-certs/my-node1.crt ~/.darkhan-certs/my-node1.key user@remote:~/.darkhan-certs/
...
rm ~/.darkhan-certs/ca.*
bash server/scripts/generate-certs.sh ~/.darkhan-certs my-primary
bash server/scripts/generate-certs.sh ~/.darkhan-certs my-node1
